...
#     x=run_df['Transmembrane Pressure drop, kPa']
#     plt.scatter(x,
#             y, 
#             marker='o',
#             s=50,
...
#     plt.plot(x_vals, fx(x_vals), label=poly_to_string(fx, "Flux", "dP"), linestyle="--")
#     r2 = calculate_r_squared(x, y, fx)
    
#     plt.plot([], [], label=f"R² = {r2:.5f}")
...
for run in unique_runs:
    ...
    plt.xlabel(r"$\Delta P$ (kPa)", fontsize=24, fontname="Arial")
    ...
    fx = np.poly1d(FluxOfP)
    x_vals = np.linspace(min(x), max(x), 1000)
    plt.plot(x_vals, fx(x_vals), label=poly_to_string(fx, r"$J_w$", r"($\Delta P$)"), lw=5, linestyle="--",zorder=3)
    r2 = calculate_r_squared(x, y, fx)
    
    plt.plot([], [], label=f"R² = {r2:.6f}", color="white")
